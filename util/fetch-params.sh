...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/USDSportsParams"
    else
        PARAMS_DIR="$HOME/.usdsports-params"
    fi
fi
...
Failed to fetch the USDSports zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
USDSports - fetch-params.sh

This script will fetch the USDSports zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common USDSports zkSNARK parameters. Note that it is
