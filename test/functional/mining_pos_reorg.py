...
from test_framework.test_framework import USDSportsTestFramework
...
class ReorgStakeTest(USDSportsTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zusdsp):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected USDSP and zUSDSP supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zUSDSPsupply']:
                assert_equal(node['zUSDSPsupply'][denom], DecimalAmt(expected_zusdsp[denom]))
    ...
    def run_test(self):
        ...
        # Check USDSP and zUSDSP supply at the beginning
        # ------------------------------------------
        # zUSDSP supply: 2 coins for each denomination
        expected_zusdsp_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # USDSP supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zusdsp_supply)
        ...
        # Verify that USDSP and zUSDSP supplies were properly updated after the spends and reorgs
        self.log.info("Check USDSP and zUSDSP supply...")
        ...
        expected_zusdsp_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zusdsp_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zusdsp_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zusdsp_supply)
