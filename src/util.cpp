...
const char * const USDSports_CONF_FILENAME = "usdsports.conf";
const char * const USDSports_PID_FILENAME = "usdsports.pid";
const char * const USDSports_MASTERNODE_CONF_FILENAME = "masternode.conf";


// USDSports only features
// Masternode
...
#else
    const char* pszModule = "usdsports";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\USDSports
// Windows >= Vista: C:\Users\Username\AppData\Roaming\USDSports
// Mac: ~/Library/Application Support/usdsports
// Unix: ~/.usdsports
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "USDSports";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\USDSportsParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\USDSportsParams
    // Mac: ~/Library/Application Support/USDSportsParams
    // Unix: ~/.usdsports-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "USDSportsParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "USDSportsParams";
#else
    // Unix
    return pathRet / ".usdsports-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", USDSports_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", USDSports_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty usdsports.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override usdsports.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", USDSports_PID_FILENAME));
