...
bool CUsdspStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CUsdspStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CUsdspStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CUsdspStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CUsdspStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CUsdspStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CUsdspStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CUsdspStake::GetUniqueness() const
{
    //The unique identifier for a USDSP stake is the outpoint
...
CBlockIndex* CUsdspStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CUsdspStake::ContextCheck(int nHeight, uint32_t nTime)
{
