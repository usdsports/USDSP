...
#include "config/usdsports-config.h"
#endif
...
#include "qt/usdsports/splash.h"
#include "qt/usdsports/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("USD Sports", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in usdsports.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in usdsports.qrc)
...
/** Class encapsulating USDSports startup and shutdown.
...
/** Main USDSports application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (USDSportsGUI)
...
{
    window = new USDSportsGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &USDSportsGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &USDSportsGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(USDSportsGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(USDSportsGUI::DEFAULT_WALLET);
        ...
        // USDSports: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &USDSportsGUI::handlePaymentRequest);
        connect(window, &USDSportsGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. USDSports can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(usdsports_locale);
    Q_INIT_RESOURCE(usdsports);
    ...
    /// 6. Determine availability of data directory and parse usdsports.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("USD Sports"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("USD Sports"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // usdsports: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("USD Sports didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
