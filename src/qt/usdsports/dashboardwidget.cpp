...
#include "qt/usdsports/dashboardwidget.h"
#include "qt/usdsports/forms/ui_dashboardwidget.h"
#include "qt/usdsports/sendconfirmdialog.h"
#include "qt/usdsports/txrow.h"
...
DashboardWidget::DashboardWidget(USDSportsGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareUsdsp, "square-chart-usdsp");
	setCssProperty(ui->labelUsdsp, "text-chart-usdsp");
	...
	setCssProperty(ui->labelAmountUsdsp, "text-stake-usdsp-disable");
	...
	if (window)
		connect(window, &USDSportsGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &USDSportsGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &USDSportsGUI::openFAQ);
	...
	// update the display unit, to not use the default ("USDSP")
	updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair USDSP
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			usdsp = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalUsdsp += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesUsdsp.append(usdsp);

		int max = std::max(usdsp, usdsp);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->setColor(QColor(238, 229, 15));
	...
	set0->append(chartData->valuesUsdsp);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalUsdsp > 0) {
		setCssProperty(ui->labelAmountUsdsp, "text-stake-usdsp");
	}
	else {
		setCssProperty(ui->labelAmountUsdsp, "text-stake-usdsp-disable");
	}
	forceUpdateStyle({ ui->labelAmountUsdsp });
	ui->labelAmountUsdsp->setText(GUIUtil::formatBalance(chartData->totalUsdsp, nDisplayUnit));
