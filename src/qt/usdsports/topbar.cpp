...
#include "qt/usdsports/topbar.h"
#include "qt/usdsports/forms/ui_topbar.h"
#include "qt/usdsports/lockunlock.h"
#include "qt/usdsports/qtutils.h"
#include "qt/usdsports/receivedialog.h"
#include "qt/usdsports/loadingdialog.h"
...
TopBar::TopBar(USDSportsGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopUsdsp}, "amount-small-topbar-main");
    setCssProperty({ui->labelAmountUsdsp}, "amount-topbar");
    setCssProperty({ui->labelPendingUsdsp, ui->labelImmatureUsdsp}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("USD Sports")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // USDSP Total
    QString totalUsdsp = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // USDSP
    // Top
    ui->labelAmountTopUsdsp->setText(totalUsdsp);
    // Expanded
    ui->labelAmountUsdsp->setText(totalUsdsp);
    ui->labelPendingUsdsp->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureUsdsp->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
