...
#include "qt/usdsports/forms/ui_snackbar.h"
#include "qt/usdsports/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(USDSportsGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &USDSportsGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
