...
#include "qt/usdsports/forms/ui_addresseswidget.h"
#include "qt/usdsports/addresslabelrow.h"
#include "qt/usdsports/addnewaddressdialog.h"
#include "qt/usdsports/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(USDSportsGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination usdspAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(usdspAdd, label.toUtf8().constData(),
