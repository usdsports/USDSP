...
#if defined(NDEBUG)
#error "USDSports cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uusdsp) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 4204800 && isPoSActive) {
		nSubsidy = 5.00 * COIN;
	}
	else if (nHeight <= 8409600 && nHeight > 4204801) {
		nSubsidy = 2.50 * COIN;
	}
	else if (nHeight <= 12614400 && nHeight > 8409601) {
		nSubsidy = 1.25 * COIN;
	}
	else if (nHeight <= 16819200 && nHeight > 12614401) {
		nSubsidy = 0.625 * COIN;
	}
	else if (nHeight <= 154679200 && nHeight > 16819201) {
...
{
    util::ThreadRename("usdsports-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> usdspInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                usdspInputs.push_back(stakeIn);
        }
        const bool hasUSDSPInputs = !usdspInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasUSDSPInputs) {
                    ...
                    for (const CTxIn& usdspIn : usdspInputs)
                        if(usdspIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasUSDSPInputs) {
                            for (const CTxIn& stakeIn : usdspInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // USDSports: We use certain sporks during IBD, so check to see if they are
