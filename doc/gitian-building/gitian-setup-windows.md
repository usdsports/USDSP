...
The build script we'll be using is contained in the USDSports github repository ([contrib/gitian-build.py](https://github.com/usdsports-project/usdsports/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the USDSports repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/USDSports-Project/USDSports/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, usdsports-detached-sigs, and usdsports GitHub repos.
