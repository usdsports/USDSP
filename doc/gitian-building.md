...
*Setup instructions for a gitian build of USDSports using a VM or physical system.*

Gitian is the deterministic build process that is used to build the USDSports
...
to the USDSports GitHub release page.
...
- [Building USDSports](#building-usdsports-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/usdsports-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/usdsports-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/usdsports-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/usdsports-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/usdsports-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/usdsports-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/usdsports-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@usdsports.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/usdsports-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/usdsports-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/usdsports-osx-*-build.assert
```
